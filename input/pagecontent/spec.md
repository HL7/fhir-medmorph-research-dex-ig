...
* When accessing MedMorph Research Data Exchange IG data, Systems SHALL interpret missing data elements within resource instances returned from API requests as data not present.
...
This specification makes significant use of [FHIR profiles]({{site.data.fhir.path}}profiling.html), search parameter definitions, and terminology artifacts to describe the content to be shared as part of MedMorph Research Data Exchange IG workflows. The IG is based on [FHIR R4]({{site.data.fhir.path}}) and profiles are listed for each interaction.
...
This IG leverages the [BulkData Access IG]({{site.data.fhir.ver.buldataIg}}/index.html) published by the HL7 FHIR Infrastructure WG for: 
...
#### Substitutable Medical Applications, Reusable Technologies (SMART) on FHIR Backend Services Authorization from Bulk Data Access IG Requirements
...
* The HDEA acting as a client **SHALL** use the [US Core Server APIs]({{site.data.fhir.ver.uscoreR4}}/CapabilityStatement-us-core-server.html) as outlined in the [Data Source Capability Statement](CapabilityStatement-medmorph-research-dex-ehr.html) to access patient data from the Data Source.
